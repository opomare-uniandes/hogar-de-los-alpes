...
| **Transactional Outbox** — el agregado y sus eventos de dominio se confirman en la MISMA transacción R2DBC (tabla `outbox_evento`); un `OutboxRelay` (en `application/`) hace *polling* de las filas con `publicado_en IS NULL`, las publica a Pulsar y las marca. Cierra la ventana en la que un fallo entre el commit y el `sendAsync` a Pulsar perdía el evento aunque el estado ya estuviera guardado (el pendiente que señalaban los docs de experimentación) | `backend/{trabajos,proveedor,trabajo-saga}/infrastructure/driven-adapters/r2dbc-postgresql/.../outbox`, `backend/{trabajos,proveedor,trabajo-saga}/application/.../config/outbox/OutboxRelay.java` |
| **Interoperabilidad/escalabilidad (escenario 4)** — ACL de entrada de partners B2B2C en instancias dedicadas del mismo jar de `integracion-service` (una por partner: `seguros-los-alpes` contrato v1, `partner-b` contrato v2), cada una con su tópico/suscripción y su ScaledObject, para que un pico en un partner no consuma la capacidad de otro ni del flujo de salida | `backend/integracion/.../entry-points/.../partner`, `deploy/k8s/apps/21-integracion-service-*.yaml`, `deploy/k8s/autoscaling/30-scaledobjects.yaml` |
